<script lang="ts">
	...
	let pdfSearchInputElement: HTMLInputElement;
	...
	const uploadPdfToSearch = async (file: File) => {
		if (!file) return;

		const formData = new FormData();
		formData.append('file', file);
		// formData.append('process', 'true'); // Default is true, so optional
		...
		try {
			const response = await fetch('http://127.0.0.1:8000/api/v1/files/', {
				method: 'POST',
				body: formData
				// Headers are not strictly needed for FormData, browser sets multipart/form-data
			});

			if (response.ok) {
				const data = await response.json();
				toast.success(`Uploaded ${file.name} successfully.`, { id: toastId });
				dispatch('pdf-uploaded', { id: data.id, filename: file.name }); // Dispatch event with ID and filename
				return data.id; // Store this ID to attach to messages
			} else {
				const errorData = await response.text(); // Get more error details if possible
				console.error('Upload failed:', response.status, errorData);
				toast.error(`Upload failed for ${file.name}: ${response.statusText} - ${errorData}`, {
					id: toastId
				});
			...
			toast.error(`Error uploading ${file.name}: ${error.message}`, { id: toastId });
		}
	};
	...
	function handlePdfFileChange(event: Event) {
		...
		if (file) {
			uploadPdfToSearch(file);
		}
		// Reset the input value to allow uploading the same file again
		if (pdfSearchInputElement) {
			pdfSearchInputElement.value = '';
		}
	}
</script>
...
<!-- Hidden file input for PDF Search -->
<input
	bind:this={pdfSearchInputElement}
	id="pdf-search-input"
	type="file"
	accept=".pdf"
...
>
	...
	<div slot="content">
		...
		>
			...
			<!-- Start: Upload to PDF Search Button -->
			<Tooltip content={$i18n.t('Upload a PDF file to the search index')} className="w-full">
				<DropdownMenu.Item
					class="flex gap-2 items-center px-3 py-2 text-sm font-medium cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800 rounded-xl"
					on:click={() => {
						pdfSearchInputElement?.click(); // Trigger hidden file input
					}}
				>
					<!-- PDF Icon -->
					...
					<div class="line-clamp-1">{$i18n.t('Upload to PDF Search')}</div>
				</DropdownMenu.Item>
			</Tooltip>
			<!-- End: Upload to PDF Search Button -->
