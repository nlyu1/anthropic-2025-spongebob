...
class StaticFilesCORS(StaticFiles):
    """Custom StaticFiles class that adds CORS headers to all responses"""
...
class SPAStaticFilesCORS(StaticFiles):
    """SPA Static Files with CORS and 404 handling for routes"""
    async def get_response(self, path: str, scope):
        try:
            response = await super().get_response(path, scope)
            # Add CORS headers
...
app.mount("/static", StaticFilesCORS(directory=STATIC_DIR), name="static")
app.mount("/cache", StaticFilesCORS(directory=CACHE_DIR), name="cache")
...
if os.path.exists(FRONTEND_BUILD_DIR):
    ...
    app.mount(
        "/",
        SPAStaticFilesCORS(directory=FRONTEND_BUILD_DIR, html=True),
